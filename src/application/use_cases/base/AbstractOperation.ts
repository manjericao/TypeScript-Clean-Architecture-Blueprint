 ...
 *
 * This class is designed to be extended and used in scenarios where event-driven
 * communication patterns are employed.
 *
 * @template EventMap A type mapping event names to their corresponding payload types.
 *
 * Type Parameters:
...
export abstract class AbstractOperation<EventMap> {
   ...
   *
   * @template {object} T - The type mapping event names to their corresponding payload types
   ...
   *
   * @template K - Type representing a key of EventMap
   * @param {K} eventName - The name of the event to emit. This should be a key from the EventMap.
   * @param {any} event - The event payload associated with the given event name.
   ...
   *
   * @template T - Type extending DomainEvent
   ...
   * @param {(event: any) => void} handler - The callback function to handle the event. Receives the event object as a parameter.
   * @return {void} This method does not return a value.
   */
   ...
   *
   * @template K
   * @param {K} eventName - The name of the event to listen for, which must be a valid key in the EventMap.
   * @return {Promise<any>} A promise that resolves with the data associated with the emitted event.
   */
